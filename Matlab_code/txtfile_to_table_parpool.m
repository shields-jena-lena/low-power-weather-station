...
% Change the 'nums', 'filename', and 'fin_name' variables as needed

clear all
close all

%list of weather station numbers to cycle through
nums = [3,4,5,6,7];
...
for j = 1:length(nums)
    ...
    % wind direction calibration from voltage readings from wind vane

    %Horizontal line values to bin the voltage readings
    t_16 = 73;
    t_15 = 87;
    t_14 = 105;
    t_13 = 150;
    ...
    % Get local variable for the analog wind direction measurements
    ...
    %Get new variables into the final table
    final_table.windPos = windPos;
    final_table.windAngle = windAngle;
    final_table.windName = windName;

    % wind speed calibration set up
    final_table = movevars(final_table,'cup_count','after','windName'); %Move cup_count to end
    
    %Get calibration values for cup count to wind speed, either from
    %datasheet or self-calibrations
    ...
    %Make local variable for cup_count, cup_count shifted by 1, cup_count
    %shifted by 10, and get the length of cup_count
    cup_count = final_table.cup_count;
    cup_count_min1 = [NaN; cup_count];
    cup_count_min10 = [NaN(10,1); cup_count];
    l = length(cup_count);
    ...
    % save file using defined fin_name from above
    save_name=[fin_name,'.mat'];
    save(save_name, "final_table")
end

%% check wind vane for each station, that the binning values work

% num = 35; %number of station of interest
% %Filename of station of interest
% filename = strcat('sample_data/weatherstation_b', num2str(num), '.txt');
% 
% %Open the .txt file, read it, and close it
% fid = fopen(filename, 'rt'); %Open the file to read it in text mode
...
% 
% 
% %Plot voltage data for wind direction
% plot(final_table.Time, final_table.wind_dir, 'o', MarkerSize=1)
% hold on
% xlabel('Time')
% ylabel('Wind analog measurement')
% 
% %Plot horizontal lines for binning
% yline([t_16 t_15 t_14 t_13 t_12 t_11 t_10 t_9 t_8 t_7 t_6 t_5 t_4 t_3 t_2 ], 'r--')
